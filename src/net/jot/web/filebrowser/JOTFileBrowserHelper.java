...
public class JOTFileBrowserHelper
{
	...
	private static void orderBy(JOTFileBrowserSession fbSession, String value)
	{
		...
		{
			if(sort==SORT_BY_NAME_ASC || sort==SORT_BY_NAME_DESC)
				sort=0-sort;
			else
		...
		{
			if(sort==SORT_BY_SIZE_ASC|| sort==SORT_BY_SIZE_DESC)
				sort=0-sort;
			else
				//default: newer first
				sort=SORT_BY_SIZE_DESC;				
		}
		else if(value.equalsIgnoreCase("timestamp"))
		{
			if(sort==SORT_BY_TSTAMP_ASC || sort==SORT_BY_TSTAMP_DESC)
				sort=0-sort;
			else
				// default: most recent first
				sort=SORT_BY_TSTAMP_DESC;
		}
         ...
         * @param fbSession
         */
	private static void sortFolderListing(JOTFileBrowserSession fbSession)
	{
		Comparator comp=JOTFileComparators.NAME_ASC_COMPARATOR;
		switch(fbSession.getSortBy())
		{
