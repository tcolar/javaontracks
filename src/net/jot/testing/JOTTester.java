...
public class JOTTester
{
				...
				for (int j = 0; j != files.length; j++)
				{
					...
					} else
					{
						...
						if (validClassPath && f.getName().endsWith(".class"))
						{
							...
							debug("** Checking class " + currentPackage + className);
							// Class is Testable ?
	...
	public boolean isClassTestable(String className)
	{
		...
		try
		{
			...
			// this throws an exception even though i try to catch it ???
			//Method m = c.getMethod(TEST_METHOD_NAME, (Class[]) null);
			Method m =null;
			Method[] methods = c.getMethods();
			for(int i=0 ;i !=methods.length; i++)
			{
				if(methods[i].getName().equalsIgnoreCase(TEST_METHOD_NAME))
					m = methods[i];
			}
		...
		for (int i = 1; i != args.length; i++)
		{
			...
			if (arg != null)
			{
				...
				if (arg.startsWith(OPTION_INCL_PK))
				{
					...
					String packages = arg.substring(OPTION_INCL_PK.length(), arg.length());
